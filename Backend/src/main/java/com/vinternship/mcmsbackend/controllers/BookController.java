...
public class BookController {
    ...
    public ResponseEntity<List<Book>> getAllBook(@RequestParam(required = false) String name) {
        try {
            ...
            if (name == null) {
                ...
                bookRepository.findBookByTitleContaining(name).forEach(books::add);
            }
 ...
 @GetMapping("/books/title/{title}")
  public ResponseEntity<List<Book>> findBookByTitleContaining(@PathVariable("title") String title) {
     try {
           List<Book> b2 = bookRepository.findBookByTitleContaining(title);
          if (b2.isEmpty()) {
               return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
